...
import {isDneroNetwork} from "../../constants/Networks";
...
function pendingDneroNetworkTransactionToLocalTransaction(network, pendingTransaction, hash) {
    ...
    return {
        ...
        fee: {
            dnero: "0",
            dtoken: transactionFee
        },
        inputs: [
            {
                address: from,
                coins: {
                    dnero: (tokenType === TokenTypes.DNERO ? amount : 0),
                    dtoken: (tokenType === TokenTypes.DNERO_TOKEN ? amount : 0) + transactionFee
                }
            }
        ],
        outputs: [
            {
                ...
                coins: {
                    dnero: (tokenType === TokenTypes.DNERO ? amount : 0),
                    dtoken: (tokenType === TokenTypes.DNERO_TOKEN ? amount : 0)
                }
            }
        ],
...
function pendingEthererumNetworkTransactionToLocalTransaction(network, pendingTransaction, hash) {
    ...
    if (pendingTransaction.tokenType === TokenTypes.ERC20_DNERO) {
        type = TokenTypes.ERC20_DNERO;
        tokenSymbol = "DNERO";
    }
...
function pendingTransactionToLocalTransaction(network, pendingTransaction, hash) {
    if (isDneroNetwork(network)) {
        return pendingDneroNetworkTransactionToLocalTransaction(network, pendingTransaction, hash);
    }
    else {
        //Unknown network
        return null;
...
export const transactionsReducer = (state = INITIAL_STATE, action) => {
    switch (action.type) {
        ...
        //Dnero Transactions
        case actionTypes.FETCH_TRANSACTIONS_DNERO_START: {
            return Object.assign({}, state, {
                isFetchingTransactions: true
            });
        }
        case actionTypes.FETCH_TRANSACTIONS_DNERO_END: {
            return Object.assign({}, state, {
                isFetchingTransactions: false
            });
        }
        case actionTypes.FETCH_TRANSACTIONS_DNERO_SUCCESS: {
            let body = action.response.body;
            let transactions = body.transactions;
        ...
        case actionTypes.FETCH_TRANSACTION_SUCCESS: {
            ...
            if(isDneroNetwork(metadata.network)){
                //The explorer may give us txs that are pending and have no data.  The UI can't show these because it has no data
                transactions = _.filter(transactions, function(tx) {
                    return (tx['inputs'] !== null);
                });
